...
for epoch in range(0,epochnum):
	...
	for st in range(0,52):
		...
		input=torch.Tensor(data[idx[batchstart]]).view(1,150000)#[1,150000]
		for iter in range(batchstart+1,batchend):
			i=idx[iter]
			#print(iter," ",i)
			lbval=label[i].index(1)
			target[iter-batchstart]=lbval
			tmp=torch.Tensor(data[i]).view(1,150000)
	...
	for i in range(0,600):
		...
		input=torch.Tensor(validdata[i]).view(1,150000)
		target=target.long()
		input=input.cuda()
		target=target.cuda()
	...
	if sumloss<bestloss*0.99:
		bestloss=sumloss
		lastbest=epoch
		torch.save(model,"mlp_"+str(baselrate)+".pth")
		issaved=True
	else:
...
file=open("mlp_valid_loss_"+str(baselrate)+".json","w")
json.dump(validloss,file)
file.close()

file=open("mlp_valid_acc_"+str(baselrate)+".json","w")

...
for epoch in range(0,epochnum):
	...
	for st in range(0,52):
		...
		input=torch.Tensor(data[idx[batchstart]]).unsqueeze(1)#[500,1,300]
		for iter in range(batchstart+1,batchend):
			i=idx[iter]
			#print(iter," ",i)
			lbval=label[i].index(1)
			target[iter-batchstart]=lbval
			tmp=torch.Tensor(data[i]).unsqueeze(1)
	...
	for i in range(0,600):
		...
		input=torch.Tensor(validdata[i]).unsqueeze(1)
		target=target.long()
		input=input.cuda()
		target=target.cuda()
		...
		if target.item()==res.item():
			accu=accu+1
	print("Epoch No.",epoch," Train Loss ",trainloss)
	...
	if sumloss<bestloss*0.99:
		bestloss=sumloss
		lastbest=epoch
		torch.save(model,"rnn_"+str(baselrate)+".pth")
		issaved=True
	else:
...
file=open("rnn_valid_loss_"+str(baselrate)+".json","w")
json.dump(validloss,file)
file.close()

file=open("rnn_valid_acc_"+str(baselrate)+".json","w")
